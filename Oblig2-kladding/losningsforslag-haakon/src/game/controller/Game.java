...
public class Game {
	...
	public void start() throws LevelNotFoundException, BuildLevelException {
		...
		while(currentLevel <= loader.getNumLevels()){
			...
			window.popupVictory();
			String hs = generateHighScore(currentLevel, timeTaken);
			window.popupGeneric("Tid brukt", hs);
			currentLevel++;
		}
		window.popupGameComplete();
	}

	private String generateHighScore(int currentLevel, long timeTaken) {
		StringBuilder highscore = new StringBuilder();
		List<Integer> scores = loader.getHighScores(currentLevel);
		if(scores.isEmpty()){
			highscore.append(timeTaken).append("<-- this is you!\n");
		}
		boolean inserted = false;
		for(Integer score : scores){
			if(score > timeTaken && !inserted){
				highscore.append(timeTaken).append("<-- this is you!\n");
				inserted = true;
			}
			highscore.append(score).append("\n");
		}
		if(!inserted){
			highscore.append(timeTaken).append("<-- this is you!\n");
		}
		loader.addHighScore(currentLevel, (int) timeTaken);
